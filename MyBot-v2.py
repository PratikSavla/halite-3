...
while True:
    ...
    for ship in me.get_ships():
        ...
        #     #print(game_map.get_unsafe_moves(ship.position, me.shipyard.position)[0])
        ...
        if ship_states[ship.id] == "collecting":
            # cardinals get surrounding cardinals using g0et_all_cardinals in positionals.py.
            ...
            #         if dn>1:
            #             #if game_map.calculate_distance(ship.position, me.get_dropoff(i).position)>game.calculate_distance(ship.position,me.shipyard.position):
            #             if random.random()>0.5:
            #                 command_queue.append(ship.move(game_map.naive_navigate(ship.position, me.get_dropoff(0))))
            #                 dp=0
            #                 break
            #     if dp ==1:
            if upcoming_position not in position_choices:
                ...
                if game_map.naive_navigate(ship, ship.position+Position(*Direction.Still)) != Direction.Still and len(game_map.get_unsafe_moves(ship.position, me.shipyard.position))>0: 
                    move = game_map.get_unsafe_moves(ship.position, me.shipyard.position)[0]
                    #upcoming_position = ship.position + Position(*move)
                    command_queue.append(ship.move(move))
    ...
    #if len(me.get_ships())<3:
    #    command_queue.append(me.shipyard.spawn())
    if (len(me.get_ships())-10) < math.ceil(game.turn_number / 50) and len(me.get_ships())<10:
