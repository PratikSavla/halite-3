...
while True:
    	...
    	if scount/ndrop >10:
	        ...
	        if ship_states[ship.id] == "collecting":
	            ...
	            if len(me.get_dropoffs())>0:
	                if game_map.calculate_distance(me.get_dropoffs()[0].position, ship.position)>game_map.calculate_distance(me.shipyard.position, ship.position):
	                    move = game_map.naive_navigate(ship, me.get_dropoffs()[0].position)
	            else:
    ...
    if len(me.get_ships()) < math.ceil(game.turn_number / 50):
