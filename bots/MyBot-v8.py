...
while True:
    ...
    for ship in me.get_ships():
        ...
        if game.turn_number>endd:
            ...
            if ddi == "ship":
                if len(game_map.get_unsafe_moves(ship.position, me.shipyard.position))>0:
                    position_options = ship.position.get_surrounding_cardinals() + [ship.position]
                    if me.shipyard.position in position_options:
                ...
                if len(game_map.get_unsafe_moves(ship.position, me.get_dropoff(ddi).position))>0:
                    position_options = ship.position.get_surrounding_cardinals() + [ship.position]
                    if me.get_dropoff(ddi).position in position_options:
                        command_queue.append(ship.move(game_map.get_unsafe_moves(ship.position, me.get_dropoff(ddi).position)[0]))
                    else:
                        command_queue.append(ship.move(game_map.naive_navigate(ship, me.get_dropoff(ddi).position)))
                else:
                    command_queue.append(ship.move(Direction.Still))
        ...
        elif len(me.get_ships())/(len(me.get_dropoffs())+1)>5 and me.halite_amount>6000 and i==0 and game_map.calculate_distance(ship.position, me.shipyard.position)>7:
...
#    if len(game_map.get_unsafe_moves(ship, me.shipyard))>0:
#        command_queue.append(ship.move(game_map.get_unsafe_moves(ship, me.shipyard)[0]))
#    else:
#        command_queue.append(ship.move(Direction.Still))